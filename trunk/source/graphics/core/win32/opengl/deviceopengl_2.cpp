...
SPBUFFER DeviceOpenGL::CreateBuffer( const CREATEBUFFERPARAM& param, const SUBRESOURCE* data )
{
  BufferOpenGL* p = new BufferOpenGL(param);

  if( data!=NULL )
  {
    p->Initialize( data->pData, data->Pitch*data->Slice );
  }else
  {
    p->Initialize();
  }
...
SPTEXTURE2D DeviceOpenGL::CreateTexture2D( const CREATERETEXTURE2DPARAM& param, const SUBRESOURCE* data )
{
  Texture2DOpenGL* p = new Texture2DOpenGL( param );
  p->Initialize( data );

  return SPTEXTURE2D(p);
...
SPRENDERTARGET DeviceOpenGL::CreateRenderTarget( const SPRESOURCE& resource, const CREATERENDERTARGETPARAM* param )
{
/*
...
SPDEPTHSTENCIL DeviceOpenGL::CreateDepthStencil( const SPRESOURCE& resource, const CREATEDEPTHSTENCILPARAM* param )
{
/*
  ID3D11Resource* pResource = NULL;
  {
...
SPMATERIAL DeviceOpenGL::CreateMaterial( const SPRESOURCE& resource, const CREATEMATERIALPARAM* param )
{
  ...
  MaterialOpenGL* pMat = new MaterialOpenGL( resource, *param );


  return SPMATERIAL(pMat);
...
SPVERTEXSHADER DeviceOpenGL::CreateVertexShader( const void* pData, size_t Length )
{
	VertexShaderOpenGL* pShader = new VertexShaderOpenGL();

	pShader->Initialize( pData, Length );
...
*/
  return SPVERTEXSHADER(pShader);
}

SPPIXELSHADER DeviceOpenGL::CreatePixelShader( const void* pData, size_t Length )
{
	PixelShaderOpenGL* pShader = new PixelShaderOpenGL();
...
*/
  return SPPIXELSHADER(pShader);
}

SPSAMPLERSTATE DeviceOpenGL::CreateSamplerState( const SAMPLERSTATEPARAM& Option )
{
/*
...
SPRASTERIZERSTATE DeviceOpenGL::CreateRasterizerState( const RASTERIZERSTATEPARAM& Option )
{
/*
...
SPDEPTHSTENCILSTATE DeviceOpenGL::CreateDepthStencilState( const DEPTHSTENCILSTATEPARAM& Option )
{
/*
...
SPBLENDSTATE DeviceOpenGL::CreateBlendState( const BLENDSTATEPARAM& Option )
{
...
*/
  return SPBLENDSTATE( new BlendStateOpenGL(Option) );
}

SPINPUTLAYOUT DeviceOpenGL::CreateInputLayout( const INPUT_ELEMENT* Element, int Count, const void* pShaderBytecodeWithInputSignature, size_t BytecodeLength )
{
  std::vector<INPUT_ELEMENT> ele( Element, Element+Count );
  InputLayoutOpenGL* p = new InputLayoutOpenGL(ele);
...
*/
  return SPINPUTLAYOUT( p );
}

bool DeviceOpenGL::CompileShaderLanguage( const String& Source, CODETYPE type, std::vector<unt08>& Binary, String& ErrorMessage )
{
  //  コンパイルする必要がありそうですが、GLSLの仕様上バイトコードがみれないので
  //  CreateVertexShader() CreatePixelShader() で作成するようになってます
	//  ここでは文字列を通過させるだけですね

  std::string SrcData = String::ConvertMAIDtoSJIS(Source);
  ...
  if( SrcData.length()<=4 )
  {
    ...
    if( 100<=no && no<=199 )
    {
      ...
      CreateDefaultVertexShader( no, SrcData );
    }
    else if( 200<=no && no<=299 ) 
    {
      //  PixelShader の場合は CreatePixelShader で作ってもらう
      CreateDefaultPixelShader( no, SrcData );
    }
    else { MAID_ASSERT( true, "このデバイスでは対応していません" ); return false; }
  }else

  if( SrcData.empty() ) { return true; }

  Binary.resize( SrcData.length() );
  memcpy( &(Binary[0]), SrcData.c_str(), SrcData.length()  );
  Binary.push_back('\0');
