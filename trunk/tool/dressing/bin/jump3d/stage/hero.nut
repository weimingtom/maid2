class Hero extends DrawObject
{
  ...
  {
    _JumpPow = 0
    y = block.GetCollision().top - Type.CenterY
  }
  ...
  function GetTopCollision()
  {
    ...
    return RECT2D(tmp.left+10, tmp.bottom-10, tmp.w-10-10, 10 );
  }

  function GetBottomCollision()
  {
    local tmp = GetCollision()
    return RECT2D(tmp.left+10, tmp.top-5, tmp.w-10-10, 10 );
  }
  ...
  {
    return RECT2D( x-Type.CenterX, y-(Type.Height-Type.CenterY), Type.Width, Type.Height )
